...
#' @param tulopvm_field The name of the variable containing the admission dates to be considered.  This is the incident date of the diagnoses related to an inpatient episode. The date must be set to integer format. 
#' @param lahtopvm_field The name of the variable containing the discharge dates to be considered. This date is considered when subsetting the diagnoses that meet the requirement for minimum age. The date must be set to integer format.
...
find_first_date <- function(dat_in, dg_field, tulopvm_field = "tulopvm_inpat_psy", 
  ...
  # some false date entries may have persisted even at this point. Hence, discard episodes that start before birth.
  d0 <- dat_in[get(tulopvm_field)> birthday]
  
  # first_any and first_any_w_f_dg without a minimum age.
  # These are the first possible dates.
  ...
  # First dates by diagnoses with minimum age considered:
  
  # first date by diagnosis with the earliest possible age of onset set in
...
# to combine results from the chunks and to get the overall first date of any disorder with
# minimum age conditions included
#'
#' @param dat_in A data.table containing one or more outputs of the find_first_date function.
#' @param one_treatment_type_only Logical. TRUE: processes a single type of data at once. FALSE: dat_in may consist of
#'    inpatient, outpatient, and primary care data that are bound by rbindlist with idcol="source".
#' @param dg_age A data.table such as dg_groups_w_min_ages from 2a_first_dates_set_diagnoses.R or similar.
#' @param id_field The name of the ID variable, "shnro" throughout.
#'
#' @return
...
combine_first_dates <- function(dat_in,one_treatment_type_only = FALSE, dg_age, id_field = "shnro"){
  ...
  # get earliest dates by person and by diagnosis ------------------------
  ...
  # if dat_in contains one treatment type only, i.e., primary care or inpatient or outpatient, source variable is not needed

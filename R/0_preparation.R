...
#   - Read functions: 
#       In case you cannot rename the raw files or organize the row data sub-folders, 
#       reading the data might be a bit laborious. These read functions represent a real-world example.
#   - Preparation functions:
#       + process_dgs_1996_2017 and process_dgs_2018_: format of diagnoses data 
#         are in slightly different format
#       + process_before_1996: years 1975-1995 are divided in several files, 
#         which need no be handled separately, but this function contains the 
#         parts that can be pre-processed in similar way (after varying pre-pre-processing)
...
# Next, the pre-processing is in the following scipts.
# Each dataset will be red and processed. The prcessed data will be saved into
# "longitudinal" chunks, meaning that each individual's all data will be saved
# into same file, using append cuntionality of fwirte function

# 0b_preparation_before_1996.R:
#   - These data contain only inpatient care and less variables then data since 1996.
...
#       Primary care data
#   - Finally, longitudinal chunks will be seved by appending.
...
# Individuals are grouped based on the first character in ID-variable (here called `shnro`).
# In the next step, each group will be processed separately for performance reasons.

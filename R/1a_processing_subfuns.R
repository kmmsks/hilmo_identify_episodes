...
# These functions contain the actual processing: identifying inpatient episdoes
...
identify_inpatient_episodes <- function(inpat_0, add_days, start_year, end_year, include_new_variables = TRUE){
  ...
  # Treshold for the highest lpvm (= end date)
...
process_outpatient_data <- function(outpat_0, inpat_1, start_year, end_year) {
  ...
  # Event number. Event refers to an inpatient episodes with possible outpatient 
  # appointments within, or to outpatient appointments outside inpatien episodes.
  ...
  # Event with different composition are aggregated separately and the aggregate events are
  # cellected to list called lst:
  
  lst <- list()
  
  # Event consist of one outpatient appointment. Do nothing except select the variables of interest:
  ...
  # Event consist of more than one outpatient appointments. Do nothing except mark these events and select the variables of interest.
    ...
    # outpat_same_day T if more than one outpaitent appointments on the same day
    
    # psy dg:t: 
    # inpat: dg_inpat_psy, outpat: dg_outpat (psyk pkl kaynnit) ja dg_outpat_psy: osas
...
process_primary_care <- function(prim_care_0, inpat_outpat_1, start_year, end_year){
  ...
  # we concider only starting days, there are very few overnight appointments and it is hard to tell
